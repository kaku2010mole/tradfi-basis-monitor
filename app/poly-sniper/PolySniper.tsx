...
type ServiceStatus = { maintenance: boolean; status: string; impact: string | null; notice: string | null; timestamp: number };
...
export default function PolySniper() {
  ...
  const [pollMs, setPollMs] = useState(25);
  ...
  useEffect(() => {
    ...
    const loadServiceStatus = async () => {
      try {
        const response = await fetch("/api/poly-sniper/status", { cache: "no-store", signal: AbortSignal.timeout(3_000) });
        const data = await response.json() as ServiceStatus;
        if (!response.ok) throw new Error("Status lookup failed");
        if (!stopped) setServiceStatus(data);
      } catch {
        if (!stopped) setServiceStatus(null);
      }
    };
    void loadServiceStatus();
    const timer = window.setInterval(() => void loadServiceStatus(), 1_000);
  ...
  useEffect(() => {
    ...
    const refreshBook = async () => {
      if (fallbackBusyRef.current || stopped) return;
      fallbackBusyRef.current = true;
      try {
        const response = await fetch(`https://clob.polymarket.com/book?token_id=${encodeURIComponent(snapshot.tokenId)}`, { cache: "no-store", signal: AbortSignal.timeout(1_000) });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const book = await response.json() as { bids?: BookLevel[]; asks?: BookLevel[] };
        const bids = (book.bids ?? []).map((level) => ({ price: Number(level.price), size: Number(level.size) })).filter((level) => Number.isFinite(level.price) && Number.isFinite(level.size)).sort((a, b) => b.price - a.price);
        const asks = (book.asks ?? []).map((level) => ({ price: Number(level.price), size: Number(level.size) })).filter((level) => Number.isFinite(level.price) && Number.isFinite(level.size)).sort((a, b) => a.price - b.price);
        if (!stopped) {
          setSnapshot((current) => current ? { ...current, bestBid: bids[0]?.price ?? current.bestBid, bestAsk: asks[0]?.price ?? current.bestAsk, bidSize: bids[0]?.size ?? current.bidSize, askSize: asks[0]?.size ?? current.askSize, timestamp: Date.now() } : current);
          setTicks((value) => value + 1);
        }
      } catch { /* WebSocket remains the primary feed. */ }
      finally { fallbackBusyRef.current = false; }
    };
    const timer = window.setInterval(() => void refreshBook(), Math.max(10, pollMs));
    return () => { stopped = true; window.clearInterval(timer); };
  }, [pollMs, snapshot?.tokenId]);
        ...
        try {
          ...
          for (const frame of frames) {
            ...
            if (frame.event_type === "book") {
              const bids = (frame.bids ?? []).map((level: BookLevel) => ({ price: Number(level.price), size: Number(level.size) })).filter((level: { price: number; size: number }) => Number.isFinite(level.price) && Number.isFinite(level.size)).sort((a: { price: number }, b: { price: number }) => b.price - a.price);
              const asks = (frame.asks ?? []).map((level: BookLevel) => ({ price: Number(level.price), size: Number(level.size) })).filter((level: { price: number; size: number }) => Number.isFinite(level.price) && Number.isFinite(level.size)).sort((a: { price: number }, b: { price: number }) => a.price - b.price);
              setSnapshot((current) => current ? { ...current, bestBid: bids[0]?.price ?? current.bestBid, bestAsk: asks[0]?.price ?? current.bestAsk, bidSize: bids[0]?.size ?? current.bidSize, askSize: asks[0]?.size ?? current.askSize, timestamp: Number(frame.timestamp) || Date.now() } : current);
              ...
              const change = (frame.price_changes ?? []).find((item: { asset_id?: string }) => item.asset_id === snapshot.tokenId);
              if (change) setSnapshot((current) => current ? { ...current, bestBid: Number(change.best_bid) || current.bestBid, bestAsk: Number(change.best_ask) || current.bestAsk, timestamp: Number(frame.timestamp) || Date.now() } : current);
            }
            setTicks((value) => value + 1);
          }
  ...
  const maintenance = serviceStatus?.maintenance === true;
  ...
  return <main className={styles.shell}>
    ...
    {maintenance && <div className={`${styles.banner} ${styles.maintenanceBanner}`}><strong>POLYMARKET MAINTENANCE REPORTED · LIVE SNIPER REMAINS ACTIVE</strong><span>{serviceStatus?.notice ?? "Public API reports HASISSUES / MAJOROUTAGE"} · high-speed market monitoring, order submission and retries continue</span></div>}
    {!maintenance && error && <div className={styles.banner}>{error} · automatic retry is active</div>}
    <div className={styles.grid}>
      <section className={styles.panel}><header><span>01 · TARGET</span><h3>Market and trigger</h3></header><div className={styles.fields}>
        ...
        <label>HTTP safety-net cadence<input type="number" min="10" step="5" value={pollMs} onChange={(event) => setPollMs(Math.max(10, Number(event.target.value)))} /><small>ms · event-driven WebSocket triggers immediately</small></label>
