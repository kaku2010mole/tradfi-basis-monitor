...
type OracleReference = { apiSymbol: string; oracle: number; updatedAt: number };
type ComparisonPoint = { t: number; pool: number; fair: number };
type Comparison = PoolQuote & { stockHkd: number | null; fairUsd: number | null; basisPct: number | null; referenceSource: string | null; referenceTime: number | null };
...
const sourceLabel = (source: Comparison["referenceSource"]) => source === "auction-price" ? "Futu auction / IEP" : source === "book-mid" ? "Futu live midpoint" : source === "close-price" ? "Futu official close" : source === "hyperliquid-oracle" ? "Hyperliquid oracle" : "Waiting for benchmark";
...
function ComparisonChart({ points, symbol }: { points: ComparisonPoint[]; symbol: string }) {
  ...
  if (points.length < 2) return <div className={styles.emptyChart}>Collecting the first synchronized pool and benchmark samples…</div>;
  return <div className={styles.chartWrap} onMouseLeave={() => setHover(null)} onMouseMove={(event) => {
    ...
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${symbol} onchain pool and benchmark price history`}>
...
export default function OnchainPoolsPage() {
  ...
  const [sheinOracle, setSheinOracle] = useState<OracleReference | null>(null);
  ...
  const loadQuotes = useCallback(async () => {
    ...
    try {
      ...
      const [poolResponse, futuResponse, oracleResponse, ...customResponses] = await Promise.all([
        fetch("/api/onchain-pools/quote?group=hk", { cache: "no-store" }),
        fetch(`/api/hk-auction/quotes?${referenceParams}`, { cache: "no-store" }),
        fetch("/api/oracle-monitor/quotes?binance=-&para=xyz%3ASHEIN", { cache: "no-store" }),
      ...
      const oraclePayload = await oracleResponse.json() as { quotes?: OracleReference[]; error?: string };
      ...
      setSheinOracle((oraclePayload.quotes ?? []).find((quote) => quote.apiSymbol === "xyz:SHEIN") ?? null);
      setErrors([...(poolPayload.errors ?? []), ...(futuPayload.errors ?? []), ...customPayloads.flatMap((item) => !item.ok && item.payload.error ? [item.payload.error] : []), ...(!futuResponse.ok && futuPayload.error ? [futuPayload.error] : []), ...(!oracleResponse.ok && oraclePayload.error ? [oraclePayload.error] : [])]);
  ...
  const comparisons = useMemo<Comparison[]>(() => {
    ...
    return pools.map((pool) => { const reference = byStock.get(pool.stockSymbol); const stockHkd = reference?.metrics.stockReferenceHkd ?? null; const oracleReference = pool.stockSymbol === "xyz:SHEIN" ? sheinOracle : null; const fairUsd = oracleReference?.oracle ?? (stockHkd !== null && fx > 0 ? stockHkd / fx : null); return { ...pool, stockHkd, fairUsd, basisPct: pool.spotPrice !== null && fairUsd !== null ? (pool.spotPrice / fairUsd - 1) * 100 : null, referenceSource: oracleReference ? "hyperliquid-oracle" : reference?.metrics.stockReferenceSource ?? null, referenceTime: oracleReference?.updatedAt ?? reference?.futu?.marketTimestamp ?? null }; }).sort((a, b) => b.basisPct === null ? -1 : a.basisPct === null ? 1 : Math.abs(b.basisPct) - Math.abs(a.basisPct));
  }, [pools, references, sheinOracle, usdHkd]);
  ...
  return <main className={styles.shell}><div className={styles.frame}>
    <header className={styles.topbar}><div><p className={styles.eyebrow}>X LAYER / XSTOCK BASIS</p><h1>Onchain Pool Monitor</h1><p>Five selected xStock pools compared with Futu shares or the matching Hyperliquid oracle.</p></div><div className={styles.topActions}><span className={poolLive ? styles.live : styles.retrying}><i />X Layer {poolLive ? "live" : "retrying"}</span><span className={futuLive || sheinOracle ? styles.live : styles.retrying}><i />Benchmarks {futuLive || sheinOracle ? "live" : "waiting"}</span><PageSwitcher active="onchain" /></div></header>

    <section className={styles.hero}><div className={styles.poolIdentity}><span>WIDEST ABSOLUTE DEVIATION</span><h2>{widest?.displayBase ?? "xSTOCKS"}</h2><p>{widest ? `${widest.name} · ${widest.stockSymbol}` : "Discovering verified activity pools"}</p></div><div className={styles.heroPrice}><span>{widest?.basisPct !== null && widest?.basisPct !== undefined && widest.basisPct < 0 ? "DISCOUNT" : "PREMIUM"}</span><strong className={widest?.basisPct !== null && widest?.basisPct !== undefined && widest.basisPct < 0 ? styles.negative : styles.positive}>{formatPct(widest?.basisPct ?? null)}</strong><small>{widest?.basisPct === null || widest === null ? "Waiting for synchronized prices" : widest.basisPct >= 0 ? "Pool rich versus benchmark" : "Pool cheap versus benchmark"}</small></div><div className={styles.heroChange}><span>LIVE COVERAGE</span><strong>{comparisons.filter((item) => item.basisPct !== null).length} / {comparisons.length || 5}</strong><small>Sorted by |premium / discount|</small></div></section>

    <section className={styles.controlBar}><label><span>USD / HKD</span><input type="number" min="1" max="20" step="0.0001" value={usdHkd} onChange={(event) => setUsdHkd(event.target.value)} /></label><div><strong>REFERENCE RULE</strong><span>Futu auction / midpoint / close for HK shares · Hyperliquid oracle for SHEINx</span></div><div><strong>REFRESH</strong><span>3 seconds · five verified defaults plus your saved pools</span></div><button className={styles.addPairButton} onClick={() => setShowAddPair((value) => !value)}>{showAddPair ? "CLOSE" : "+ ADD PAIR"}</button></section>
    ...
    <section className={styles.poolSection}><div className={styles.sectionHead}><div><p className={styles.eyebrow}>SELECTED X LAYER POOLS</p><h2>Premium & discount ranking</h2></div><span>Select a row to inspect its synchronized history</span></div><div className={styles.poolTable}><div className={styles.poolTableHead}><span>PAIR / BENCHMARK</span><span>ONCHAIN</span><span>REFERENCE</span><span>FAIR · USD</span><span>PREMIUM / DISCOUNT</span><span>UPDATED</span></div>{comparisons.map((item) => <button key={item.id} className={`${styles.poolRow} ${selected?.id === item.id ? styles.selectedRow : ""}`} onClick={() => setSelectedId(item.id)}><span><b>{item.displayBase}</b><small>{item.name} · {item.stockSymbol}</small></span><span><b>{formatPrice(item.spotPrice, 5)}</b><small>{item.quoteSymbol} · {item.feePct.toFixed(2)}%</small></span><span><b>{item.stockHkd !== null ? `HK$${formatPrice(item.stockHkd, 3)}` : `$${formatPrice(item.fairUsd, 5)}`}</b><small>{sourceLabel(item.referenceSource)}</small></span><span><b>{formatPrice(item.fairUsd, 5)}</b><small>{item.stockHkd !== null ? `HKD ÷ ${formatPrice(Number(usdHkd), 4)}` : "Oracle · USD"}</small></span><span className={item.basisPct === null ? "" : item.basisPct >= 0 ? styles.premiumCell : styles.discountCell}><b>{formatPct(item.basisPct)}</b><small>{item.basisPct === null ? "WAITING" : item.basisPct >= 0 ? "SELL POOL / BUY BENCHMARK" : "BUY POOL / SELL BENCHMARK"}</small></span><span><b>{formatTime(item.referenceTime)}</b><small>{shortAddress(item.poolAddress)}</small></span></button>)}</div></section>

    {selected && <><section className={styles.executionStrip}><div><span>BUY {selected.baseSymbol}</span><strong>{formatPrice(selected.buyPriceBeforeSlippage, 6)}</strong><small>{selected.quoteSymbol} · fee included, before slippage/gas</small></div><div className={styles.spotCell}><span>{selected.basisPct !== null && selected.basisPct < 0 ? "DISCOUNT" : "PREMIUM"} VS BENCHMARK</span><strong className={selected.basisPct !== null && selected.basisPct < 0 ? styles.negative : styles.positive}>{formatPct(selected.basisPct)}</strong><small>{selected.basisPct === null ? "Waiting for synchronized prices" : selected.basisPct >= 0 ? "SELL POOL / BUY BENCHMARK" : "BUY POOL / SELL BENCHMARK"}</small></div><div><span>SELL {selected.baseSymbol}</span><strong>{formatPrice(selected.sellPriceBeforeSlippage, 6)}</strong><small>{selected.quoteSymbol} · fee included, before slippage/gas</small></div></section>
    <section className={styles.metrics}><article><span>POOL TVL · APPROX.</span><strong>${formatCompact(selected.tvlQuote)}</strong><small>Both token balances at pool spot</small></article><article><span>{selected.baseSymbol} IN POOL</span><strong>{formatCompact(selected.baseBalance)}</strong><small>{shortAddress(selected.baseAddress)}</small></article><article><span>BENCHMARK</span><strong>{selected.stockHkd !== null ? `HK$${formatPrice(selected.stockHkd, 3)}` : `$${formatPrice(selected.fairUsd, 5)}`}</strong><small>{sourceLabel(selected.referenceSource)}</small></article><article><span>BLOCK FRESHNESS</span><strong>{latestBlockAge === null ? "—" : `${Math.round(latestBlockAge / 1000)}s`}</strong><small>Block {Number(selected.blockNumber).toLocaleString()}</small></article></section>
    <section className={styles.chartPanel}><div className={styles.panelHead}><div><p className={styles.eyebrow}>SERVER HISTORY · {selected.displayBase}</p><h2>Pool versus benchmark USD</h2></div><div className={styles.chartLegend}><span><i className={styles.poolLegend} />ONCHAIN</span><span><i className={styles.futuLegend} />BENCHMARK</span></div><div className={styles.rangeStats}><span>LOW <b>{formatPrice(selectedRange?.low ?? null, 5)}</b></span><span>HIGH <b>{formatPrice(selectedRange?.high ?? null, 5)}</b></span></div></div><ComparisonChart points={selectedHistory} symbol={selected.displayBase} /><footer><span>Render captures every minute on weekdays, 10:00–15:00 HKT · retained for 90 days</span><span>{selectedHistory.length.toLocaleString()} saved points · hover for exact values</span></footer></section>
