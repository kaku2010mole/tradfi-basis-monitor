...
const capture = async (origin: string, cookie: string) => {
  ...
  const params = new URLSearchParams({ usdhkd: "7.84" });
  FUTU_PAIRS.forEach(([stock, perp]) => params.append("pair", `${stock}|${perp}|1`));
  const headers = cookie ? { Cookie: cookie } : undefined;
  const [poolResponse, futuResponse, oracleResponse] = await Promise.all([
    ...
    fetch(new URL("/api/oracle-monitor/quotes?binance=-&para=xyz%3ASHEIN", origin), { cache: "no-store", headers, signal: AbortSignal.timeout(20_000) }),
  ]);
  ...
  const oraclePayload = oracleResponse.ok ? await oracleResponse.json() as { quotes?: Array<Record<string, unknown>> } : { quotes: [] };
  const sheinOracle = positive((oraclePayload.quotes ?? []).find((quote) => quote.apiSymbol === "xyz:SHEIN")?.oracle);
  ...
  const records = (poolPayload.quotes ?? []).flatMap((pool) => {
    ...
    const isShein = pool.stockSymbol === "xyz:SHEIN";
    const stockHkd = positive(metrics?.stockReferenceHkd);
    const fairUsd = isShein ? sheinOracle : null;
    if (poolPrice === null || (stockHkd === null && fairUsd === null) || typeof pool.id !== "string") return [];
    return [{ poolId: pool.id, t: minute, pool: poolPrice, stockHkd, fairUsd }];
  });
  ...
  try {
    ...
    files.forEach((text) => text.split("\n").forEach((line) => {
      ...
      try {
        ...
        const storedFairUsd = positive(raw.fairUsd);
        const fair = storedFairUsd ?? (stockHkd === null ? null : stockHkd / usdHkd);
        if (!poolId || t === null || pool === null || fair === null || t < start || t > end) return;
        const points = byPool.get(poolId) ?? new Map();
        points.set(t, { t, pool, fair });
