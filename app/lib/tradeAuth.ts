export const TRADE_COOKIE = "oracle_trade_access";
...
export const tradeTokenFromRequest = (request: Request) => {
  const cookie = request.headers.get("cookie") ?? "";
  return cookie.split(";").map((part) => part.trim()).find((part) => part.startsWith(`${TRADE_COOKIE}=`))?.slice(TRADE_COOKIE.length + 1);
};
