...
async function handleFutuIngest(request: Request, configuredToken?: string, sitePassword?: string) {
  ...
  if (
    ...
    payload.quotes.some((quote) => typeof quote?.symbol !== "string" || !/^(?:HK\.(?:\d{5}|800000|HSImain)|US\.(?:LNVGY|NVDA))$/.test(quote.symbol)) ||
    ...
    (payload.history !== undefined && (
      ...
      Object.entries(payload.history).some(([symbol, points]) =>
        !/^(?:HK\.(?:\d{5}|800000|HSImain)|US\.(?:LNVGY|NVDA))$/.test(symbol) || !Array.isArray(points) || points.length > 1500 ||
